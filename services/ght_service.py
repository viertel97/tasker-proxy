	...
	if "last_answers_to_show" in df.columns:
		...
		for index, row in df.query("last_answers_to_show.notnull() and active == 1").iterrows():
			...
			for answer in answer_list:
				latest_answer = unique_ght_answers.query(f'ght_code == "{answer}"').iloc[0]
...
def handle_rework_events(df):
	...
	df = df.merge(exploded_rework_events, how="left", left_on="code", right_on="questions")
...
def get_exercises(type):
	connection = create_server_connection()
	df = pd.read_sql(
		"SELECT * FROM exercises where type = '{type}' and active = 1".format(type=type),
		connection,
	)
	close_server_connection(connection)
	return_list = []
	df.apply(
		lambda row: [return_list.append(row["exercise"]) for x in range(0, row["quantifier"])],
		axis=1,
	)
	return return_list
...
def create_obsidian_file_from_ght_data(df: pd.DataFrame, ght_type: str, timestamp: datetime):
	...
	add_files_to_repository(
		[file],
		f"Added GHT file for {timestamp.strftime('%Y-%m-%d %H:%M:%S')}",
		f"0300_Spaces/GHT/{timestamp.strftime('%Y')}/{timestamp.strftime('%m-%B')}/",
	)
