...
def update_koreader_tables(file_path):
    ...
    df_page_stat_data = pd.read_sql_query("SELECT * FROM page_stat_data", sqlite_connection)
...
def update_table_with_dataframe(table_name, new_data, unique_columns, engine):
    ...
    merged_data = new_data.merge(existing_data, on=unique_columns, how='left', indicator=True)
